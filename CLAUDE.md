...
- **작업 완료 후 결과 보고는 반드시 `report-writer` 에이전트가 작성한다.** Claude(나)가 직접 결과를 요약하거나 보고서를 쓰지 않는다.
- Claude(나)는 사용자 대화 창구 역할만 한다. 직접 에이전트를 선택하거나 작업을 분배하지 않는다.
- 흐름: `사용자 요청 → cto-lead → 전문 에이전트들 (병렬/순차) → report-writer → 사용자`
