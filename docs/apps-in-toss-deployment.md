...
[기존] 사용자 브라우저
    ↓
spotly.website (Next.js, SSR) ← 변경 없음
...
| 연결 해제 콜백 URL | `https://spotly.website/api/auth/toss/unlink` |
...
| 플랫폼 | 결제 수단 | 비고 |
|--------|----------|------|
| 웹 (spotly.website) | 토스페이먼츠 | 카드, 간편결제 |
...
if (navigator.share) {
  await navigator.share({
    ...
    url: `https://spotly.website/report/${id}`,
  });
}
```
